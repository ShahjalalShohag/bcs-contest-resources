...
vector<ll> gen(const vi& pmap, int n) {
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; ++i) {
            pmap_a[i] = powmod(35, a[i], MOD1) ^ powmod(57, a[i], MOD1);
            pmap_b[i] = powmod(35, b[i], MOD1) ^ powmod(57, b[i], MOD1);
        }
        auto ha = gen(pmap_a, n);
        auto hb = gen(pmap_b, n);

        map<ll, int> cnt_a, cnt_b;
