...
ll solve(int i) {
    ...
    ll &ans = dp[i];
    if(~ans) return ans;
    int next_i = next_high[i];
    ans = 1LL * (v[i] + 1) * (next_i - i);
    if(next_i < n && v[next_i] == v[i] + 1) 
        ans += solve(next_i);
    else ans += 1LL * (v[i] + 1) * (n - next_i);
    return ans;
}
int32_t main() {
