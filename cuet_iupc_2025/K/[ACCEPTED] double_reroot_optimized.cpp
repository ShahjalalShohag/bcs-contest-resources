...
static inline void mult(long long &a, long long b) {
    a = (a * b) % mod;
}

static inline void divide(long long &a, long long b) {
    a = (a * inverse(b)) % mod;
}
...
static inline void reroot(int x, int u) {
    // reroot from x to u
    ...
    mult(x_ways, inv_fac[sub[x] - 1]);
    mult(x_ways, fac[sub[x] - sub[u] - 1]);

    mult(u_ways, x_ways);
    // divide(u_ways, fac[sub[x] - sub[u]]);
    mult(u_ways, inv_fac[sub[x] - sub[u]]);
    // divide(u_ways, fac[sub[u] - 1]);
    mult(u_ways, inv_fac[sub[u] - 1]);
