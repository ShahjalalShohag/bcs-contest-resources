...
int main()
{
    ...
    while (T--) {
        ...
        for(int i = 0; i < n - 1; i++) {
            if(i - l[i] >= 0 && lca.depth[f[i]] == lca.depth[f[i - l[i]]]) {
                l[i] += l[i - l[i]];
            }
        }
        ...
        for(int i = 0; i < f.size(); i++) {
            ...
            for(int j = 1; j < r[i]; j++) {
                ans[left_idx] += (ll)f[i] * min(j, min_seg);
                ans[left_idx + 1] -= (ll)f[i] * min(j, min_seg);
                left_idx++;

                ans[right_idx] += (ll)f[i] * min(j, min_seg);
                ans[right_idx + 1] -= (ll)f[i] * min(j, min_seg);
                right_idx--;
            }
