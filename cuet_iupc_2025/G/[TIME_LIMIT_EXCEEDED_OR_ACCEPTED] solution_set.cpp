...
int main()
{
    ...
    while (T--) {
        ...
        for(int i = 1, j = 0; i < d.size(); i++) {
            ...
            while(j != i) {
                set<int>::iterator it = os.find(d[j].second);
                if(it == os.begin()) l[d[j].second] = d[j].second + 1;
                else {
                    it--;
                    l[d[j].second] = d[j].second - *it;
                    it++;
                }

                it++;
                if(it == os.end()) r[d[j].second] = n - d[j].second - 1;
                else {
                    r[d[j].second] = *it - d[j].second;
                }
                j++;
            }
        }
