...
int main()
{
    ...
    while (T--) {
        ...
        for(int i = 1, j = 0; i < d.size(); i++) {
            os.insert(d[i-1].second);
            if(d[i].first == d[i-1].first) {
                continue;
            }

            while(j != i) {
                int pos = os.order_of_key(d[j].second);
                if(!pos) l[d[j].second] = d[j].second + 1;
                else {
                    int left_idx = *os.find_by_order(pos - 1);
                    l[d[j].second] = d[j].second - left_idx;
                }

                if(pos == os.size() - 1) r[d[j].second] = n - d[j].second - 1;
                else {
                    int right_idx = *os.find_by_order(pos + 1);
                    r[d[j].second] = right_idx - d[j].second;
                }
                j++;
            }
        }
