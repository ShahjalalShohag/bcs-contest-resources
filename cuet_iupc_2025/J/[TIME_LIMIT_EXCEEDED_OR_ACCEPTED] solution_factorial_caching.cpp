 ...
 * Solution using: Fenwick Tree (BIT) + std::set + Naive Dynamic Calculation + Caching
 * Complexity: O(Sum(Values)) worst case once, then O((N + Q) * log N) amortized
 */
...
// Cache to store heavy factorial results (e.g., (12!)! % MOD)
map<int, int> large_fact_cache;
            ...
            while (it != active_indices.end() && *it <= r)
            {
                ...
                else if (seed > 12)
                {
                    ...
                    if (seed < MAX_N) {
                        ...
                        // Check cache first to avoid recomputing heavy factorials
                        if (large_fact_cache.count(seed)) {
                            new_val = large_fact_cache[seed];
                        } 
                        else {
                            // Naive Computation: Calculate factorial from MAX_N up to seed.
                            // For seed = 12! (479,001,600), this loop runs ~4.78*10^8 times.
                            // Only runs once per unique large seed due to caching.
                            long long val = fact[MAX_N - 1];
                            for (int k = MAX_N; k <= seed; k++) {
                                val = (val * k) % MOD;
                            }
                            new_val = (int)val;
                            large_fact_cache[seed] = new_val;
                        }
                    }
